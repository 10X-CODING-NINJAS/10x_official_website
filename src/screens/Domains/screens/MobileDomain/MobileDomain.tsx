          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-4 md:gap-6 lg:gap-8 place-items-center">
            {domainCards.map((card) => (
              <div key={card.title} className="w-full max-w-[280px] sm:max-w-[240px] md:max-w-[280px]">
                <Card className="relative w-full h-72 sm:h-64 md:h-72 lg:h-80 bg-transparent border-none overflow-hidden group hover:scale-105 transition-all duration-300 cursor-pointer">
                  <CardContent className="p-0 h-full relative">
                    ...
                    {/* Title with enhanced orange glow - matching desktop */}
                    <div className="absolute top-4 sm:top-3 md:top-4 left-1/2 transform -translate-x-1/2 z-20">
                      <h3 className="font-['Bruno_Ace'] text-white text-sm sm:text-base md:text-lg lg:text-xl font-normal tracking-tight text-center whitespace-nowrap [text-shadow:0_0_15px_rgba(255,82,0,1),0_0_30px_rgba(255,82,0,0.8),0_0_45px_rgba(255,82,0,0.6)]">
                        {card.title}
                      </h3>
                    </div>
                    
                    {/* Main image */}
                    <div className="absolute inset-0 flex items-center justify-center pt-12 sm:pt-10 md:pt-12 lg:pt-14 pb-4 px-4">
                      <img
                        className="max-w-[70%] max-h-[70%] sm:max-w-[75%] sm:max-h-[75%] md:max-w-[70%] md:max-h-[70%] object-contain transition-transform duration-300 group-hover:scale-110"
                        alt={card.imageAlt}
                        src={card.image}
