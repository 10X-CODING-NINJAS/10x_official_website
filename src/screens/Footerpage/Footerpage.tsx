...
export const Footerpage = (): JSX.Element => {
  return (
  <div className="w-screen min-h-screen overflow-hidden bg-[#000000] flex flex-col items-stretch justify-start py-4 relative footer-full-bleed">
    {/* Full-width inner content (no scale) */}
  ...
  <img className="w-28 md:w-[160px] h-16 md:h-[80px] object-contain" alt="10x logo" src="./Footer/10X Logo.svg" />
      ...
      {/* Main content row */}
  <div className="w-full flex flex-col md:flex-row justify-between items-center md:items-stretch flex-1 px-6 md:px-12 mt-3 md:mt-8 gap-6">
        {/* Mobile: nav centered above ninja, desktop: left */}
  <nav className="flex flex-col gap-2 md:gap-3 w-full md:w-auto items-center md:items-start order-1 md:order-none">
          {/* Updated routes with hash anchors for smooth scrolling to sections on home */}
        ...
        {/* Center ninja gif always centered */}
  <div className="flex flex-col items-center justify-center flex-1 py-3 order-2 md:order-none">
        ...
        {/* Mobile: domains centered below ninja, desktop: right */}
  <section className="flex flex-col items-center md:items-end gap-2 md:gap-3 w-full md:w-auto order-3 md:order-none mt-2 md:mt-0">
